	...
	async run(event: WorkflowEvent<ResearchType>, step: WorkflowStep) {
		...
		try {
			...
			const report = await step.do("generate report with search", async () => {
				...
				// Access sources directly from the return value, as shown in the documentation
				...
				• Avoid repetition, speculation, and unnecessary commentary
				• Ensure accessibility while maintaining depth
			
				IMPORTANT: DO NOT include a sources section in your response. I will add one automatically using the metadata from your search results. For each fact or claim from external sources, please mention the source name in the text (e.g., "According to Amnesty International...").
			
				Based on this framework, provide a comprehensive, well-structured report that thoroughly addresses the topic.`,
				});

				// Log both the direct sources and metadata for debugging
				console.log("Direct sources from response:", JSON.stringify(sources, null, 2));
